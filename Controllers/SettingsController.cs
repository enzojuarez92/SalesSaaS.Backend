...
public sealed record UpdateTenantUserRequest(Guid Id, Guid TenantId, [Required, StringLength(100)] string FirstName, [Required, StringLength(100)] string LastName, [Required, RegularExpression("^(Owner|Admin|Seller|Warehouse)$")] string Role);
...
public sealed class UsersController(ApplicationDbContext context, ISender sender) : ControllerBase
{
    ...
    [HttpPut("{id:guid}")] public async Task<TenantUserDto> Update(Guid id, UpdateTenantUserRequest request) { if(id!=request.Id) throw new InvalidOperationException("El usuario no coincide."); var m=await context.TenantMemberships.Include(x=>x.User).SingleAsync(x=>x.UserId==id&&x.TenantId==request.TenantId); if (m.Role == Roles.Owner || request.Role == Roles.Owner) throw new InvalidOperationException("La titularidad del negocio no se cambia desde este formulario."); m.User!.FirstName=request.FirstName.Trim();m.User.LastName=request.LastName.Trim();m.Role=request.Role;await context.SaveChangesAsync();return new(m.UserId,m.User.FirstName,m.User.LastName,m.User.Email,m.Role,m.IsActive&&m.User.IsActive); }
