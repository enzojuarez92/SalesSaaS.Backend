...
public sealed record GetCustomerStatementQuery(Guid TenantId, Guid CustomerId) : IRequest<CustomerStatementDto>, ITenantScopedRequest;
...
public sealed record RecordCustomerPaymentCommand(Guid TenantId, Guid CustomerId, decimal Amount, string Description) : IRequest<Guid>, ITenantScopedRequest;
...
public sealed class GetCustomerStatementQueryHandler(ApplicationDbContext context) : IRequestHandler<GetCustomerStatementQuery, CustomerStatementDto>
{
    public async Task<CustomerStatementDto> Handle(GetCustomerStatementQuery request, CancellationToken cancellationToken)
    {
        ...
        var entries = await context.CustomerAccountEntries.AsNoTracking().Where(item => item.TenantId == request.TenantId && item.CustomerId == request.CustomerId)
        ...
        return new CustomerStatementDto(customer.Id, customer.Name, customer.CurrentBalance, customer.CreditLimit, Math.Max(0, customer.CreditLimit - customer.CurrentBalance), entries);
    }
}
...
public sealed class RecordCustomerPaymentCommandHandler(ApplicationDbContext context) : IRequestHandler<RecordCustomerPaymentCommand, Guid>
{
    public async Task<Guid> Handle(RecordCustomerPaymentCommand request, CancellationToken cancellationToken)
    {
        var customer = await context.Customers.SingleOrDefaultAsync(item => item.Id == request.CustomerId && item.TenantId == request.TenantId, cancellationToken)
            ?? throw new InvalidOperationException("El cliente no existe.");
        if (request.Amount > customer.CurrentBalance) throw new InvalidOperationException("El pago no puede superar el saldo pendiente.");
        var entry = new CustomerAccountEntry { Id = Guid.NewGuid(), TenantId = request.TenantId, CustomerId = request.CustomerId, Type = CustomerAccountEntryType.Credit, Amount = request.Amount, Description = request.Description.Trim() };
        ...
        var activeCashSession = await context.CashRegisterSessions
            .Where(session => session.TenantId == request.TenantId && session.Status == "Open")
            .OrderByDescending(session => session.OpenedAtUtc).FirstOrDefaultAsync(cancellationToken);
        if (activeCashSession is not null)
