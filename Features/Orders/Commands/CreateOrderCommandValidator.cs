...
public sealed class CreateOrderCommandValidator : AbstractValidator<CreateOrderCommand>
{
    public CreateOrderCommandValidator()
    {
        ...
        RuleFor(command => command.DiscountAmount).GreaterThanOrEqualTo(0).WithMessage("El descuento no puede ser negativo.");
        RuleFor(command => command.PaymentMethod).IsInEnum().WithMessage("El medio de pago no es válido.");
